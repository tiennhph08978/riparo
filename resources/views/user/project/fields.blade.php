<div class="content-project">
    @csrf
    <div class="import-image row @error('image_banner') input-invalid @enderror">
    ...
    <p class="text_note">{{ trans('project.text_note') }}</p>
    ...
    <div class="form-group">
        <div class="row">
            <div class="col-xl-2 col-6 @error('target_amount') input-invalid @enderror">
                ...
                <div class="row recruitment-number-money">
                    <div class="col-12">
                        <input name="target_amount"
                               ...
                               value="{{ (old('target_amount') ? str_replace(',', '', old('target_amount')) : str_replace(',', '', @$project->target_amount)) }}">
            ...
            <div class="col-xl-2 col-6 contract_period_reach">
                    ...
                    <div class="col-sm-6 col-xl-4 input-stock-distribution">
                        <div class="input-group flex-column">
                            <input type="number"
                                   ...
                                   min="0"
                                   max="100"
                                   class="form-control change-recruitment arrows number_share @error('recruitment_number') input-invalid @enderror"
    ...
    <div class="form-group">
        <label for="">{{ trans('project.attribute.available_date_work') }}<span class="required">{{ trans('common.required') }}</span></label>
        <div class="text-notice">{{ trans('project.attribute.set_time_meeting') }}</div>
        <div id="targetTable">
            @if ($project)
                @foreach($project->availableDateEdit as $date)
                    @include('user.project.default_worktime_row', ['dataInput' => $date])
                @endforeach
            @elseif(old('available_date'))
                @foreach(old('available_date') as $key => $date)
                    @include('user.project.default_worktime_row', ['dataInput' => ['id' => old('available_id')[$key], 'date' => $date, 'startTime' => old('available_start_time')[$key], 'endTime' => old('available_end_time')[$key]]])
                @endforeach
            @else
        ...
        <div id="button-add-available-date" class="row col-xl-12 col-12">
            <div class="title-candidate"></div>
            <div class="img-date-work icon-add" id="addRowDefault">
                <img src="{{ asset('assets/icon/icon_add.svg') }}" alt="">
            </div>
        </div>
    </div>
    <div class="show-sp text-center">
        <button type="submit" class="btn btn-success submit-form-data">{{ trans('common.button.confirm_content') }}</button>
    </div>
</div>
